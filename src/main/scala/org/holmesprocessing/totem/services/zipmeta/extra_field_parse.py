...
class HeaderIdMapping():
    ...
    def HeaderIds(self):
        ...
        return {
            ...
            "\x07\x00": {"name": "AV Info", "parseField": UnknownExtraField},
            "\x09\x00": {"name": "OS/2", "parseField": OS2},
            "\x0a\x00": {"name": "NTFS", "parseField": NTFS},
            "\x0c\x00": {"name": "OpenVMS", "parseField": UnknownExtraField},
            "\x0d\x00": {"name": "Unix", "parseField": Unix},
            "\x0f\x00": {
                "name": "Patch Descriptor",
                "parseField": UnknownExtraField,
            },
            "\x14\x00": {
                "name": "PKCS#7 Store for X.509 Certificates",
                "parseField": UnknownExtraField,
            },
            "\x15\x00": {
                "name": "X.509 Certificate ID and Signature for individual file",
                "parseField": UnknownExtraField,
            },
            #     ##        "\x16\x00":    { "name": "X.509 Certificate ID for Central Directory",
            #     ##                         "parseField": x_509},
            "\x65\x00": {
                "name": "IBM S/390 attributes ? uncompressed",
                "parseField": UnknownExtraField,
            },
            "\x66\x00": {
                "name": "IBM S/390 attributes ? compressed",
                "parseField": UnknownExtraField,
            },
            "\xc8\x07": {"name": "Macintosh", "parseField": UnknownExtraField},
            "\x05\x26": {
                "name": "ZipIt Macintosh",
                "parseField": UnknownExtraField,
            },
            "\x05\x27": {
                "name": "ZIpIt Macintosh 1.3.5+",
                "parseField": UnknownExtraField,
            },
            "\x4d\x33": {
                "name": "Info-ZIP Macintosh",
                "parseField": UnknownExtraField,
            },
            "\x41\x43": {"name": "Acorn/SparkFS", "parseField": UnknownExtraField},
            "\x53\x44": {
                "name": "Windows NT security descriptor (binary ACL)",
                "parseField": WindowsNTSecurityDescriptor,
            },
            "\x04\x47": {"name": "VM/CMS", "parseField": UnknownExtraField},
            "\x0f\x47": {"name": "MVS", "parseField": UnknownExtraField},
            "\x54\x48": {
                "name": "FWKCS MD5 (see below)",
                "parseField": UnknownExtraField,
            },
            "\x41\x4c": {
                "name": "OS/2 access control list (text ACL)",
                "parseField": OS2ACL,
            },
            "\x49\x4d": {
                "name": "Info-ZIP OpenVMS",
                "parseField": UnknownExtraField,
            },
            "\x4c\x4f": {
                "name": "Xceed original location extra field",
                "parseField": UnknownExtraField,
            },
            "\x56\x53": {"name": "AOS/VS (ACL)", "parseField": UnknownExtraField},
            "\x55\x54": {
                "name": "extended timestamp",
                "parseField": ExtendedTimeStamp,
            },
            "\x55\x58": {
                "name": "Info-ZIP Unix (original,}, also OS/2,}, NT,}, etc)",
                "parseField": InfoZipUnixOld,
            },
            "\x55\x4e": {
                "name": "Xceed unicode extra field",
                "parseField": UnknownExtraField,
            },
            "\x42\x65": {
                "name": "BeOS(BeBox, PowerMac, etc.)",
                "parseField": UnknownExtraField,
            },
            "\x6e\x75": {"name": "ASi Unix", "parseField": ASiUnix},
            "\x55\x78": {
                "name": "Info-ZIP Unix (previous new)",
                "parseField": Unix2,
            },
            "\x75\x78": {
                "name": "Info-ZIP Unix (new)",
                "parseField": InfoZipUnixNew,
            },
            "\x4a\xfb": {"name": "SMS/QDOS", "parseField": UnknownExtraField},
            "Unknown": {"name": "UnknownHeader", "parseField": UnknownExtraField},
        }
...
class NTFS():  # Same For Local Directory
    ...
    def parse(self, extraField, _zip64Flag):
        ...
        parsedBlock = {
            "Name": "NTFS",
            "BlockTag": binascii.hexlify(extraField[:2]),
        ...
        parsedBlock[f"tag{str(attributeTag)}"] = {
    ...
    def parse(self,extraField,_zip64Flag):
        ...
        return {
            ...
            "ATime": datetime.fromtimestamp(
                struct.unpack("<I", extraField[4:8])[0]
            ).strftime("%B %d, %Y %H:%M:%S.%f"),
            "MTime": datetime.fromtimestamp(
                struct.unpack("I", extraField[8:12])[0]
            ).strftime("%B %d, %Y %H:%M:%S.%f"),
            "Userid": struct.unpack("H", extraField[12:14])[0],
            "Groupid": struct.unpack("H", extraField[14:16])[0],
            "DataField": binascii.hexlify(
                extraField[16 : (16 + (struct.unpack("<H", extraField[2:4]))[0])]
            ),
        }
...
class Zip64Extended():
    ...
    #Zip64 Flags are true if data could not fit in Central Directory Bytes
        ...
        if zip64Flags["cZip64"]:
            parsedBlock["CompressedSize"] = struct.unpack("<Q", extraField[start:start + 8])[0],
            start += 8
